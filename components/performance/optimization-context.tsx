...
interface OptimizationContextType {
  ...
  updateSetting: (key: keyof OptimizationSettings, value: boolean | string) => void;
...
export function OptimizationProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const detectCapabilities = () => {
      ...
      if ('connection' in navigator) {
        ...
        hasSlowConnection = connection.effectiveType === 'slow-2g' ||
      ...
      if ('memory' in performance) {
        ...
        if (totalMemory < 50 * 1024 * 1024) { // Less than 50MB
          memoryStatus = 'low';
        } else if (totalMemory > 200 * 1024 * 1024) { // More than 200MB
          memoryStatus = 'high';
        }
      }
  ...
  useEffect(() => {
    if (settings.performanceMode === 'auto') {
      ...
      if (deviceCapabilities.prefersReducedMotion && settings.respectMotionPreference) {
        autoSettings.reduceAnimations = true;
        autoSettings.simplifyAnimations = true;
      }
  ...
  }, [deviceCapabilities, settings.performanceMode, settings.respectMotionPreference]);
  ...
  const updateSetting = (key: keyof OptimizationSettings, value: boolean | string) => {
...
export function useAnimationOptimization() {
  ...
  return {
    reduceAnimations: settings.reduceAnimations || (deviceCapabilities.prefersReducedMotion && settings.respectMotionPreference),
    useGPUAcceleration: settings.useGPUAcceleration && !deviceCapabilities.isLowEnd,
