...
function mapFourthwallToUnified(product: FourthwallProduct): UnifiedProduct {
  ...
  return {
    ...
    rawProduct: product, // Store raw product for modal
  };
}
...
export default function UnifiedShopSection({
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const mappedDigitalProducts = digitalProducts
        .map(p => {
          if ("preview_url" in p) {
            return mapGumroadToUnified(p as GumroadProduct);
          }
          if ("buy_now_url" in p) {
  ...
  return (
    <section id="shop" className="relative w-full overflow-hidden py-0">
      {/* Atmospheric decorative glow */}
      <div className="absolute bottom-0 right-0 h-[600px] w-[600px] translate-x-1/4 translate-y-1/4 rounded-full bg-primary/5 blur-[150px]" />

      <div className="container relative z-10 mx-auto max-w-6xl px-6">
        {/* Header - Pixel & Minimal */}
        <div className="mb-8 space-y-4 text-center">
          <span className="font-pixel text-sm font-bold uppercase tracking-[0.2em] text-primary/80">
            The Vault
          </span>
          <h2 className="font-pixel text-4xl font-normal tracking-tight text-foreground md:text-4xl">
            Official Merch
          </h2>
        </div>
        ...
        {loading && (
          <div className="grid grid-cols-1 gap-12 pt-12 md:grid-cols-2">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="glass-card h-[450px] animate-pulse rounded-[2.5rem]"
              />
            ))}
          </div>
        )}
          ...
          <div className="grid grid-cols-1 gap-16 md:grid-cols-2 lg:grid-cols-3">
            {products.map(product => (
              <ProductCard
                ...
                product={product}
                rawProduct={product.rawProduct as FourthwallProduct | undefined}
              />
            ))}
          </div>
        )}
