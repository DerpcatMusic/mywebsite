...
export function getPatreonTierUrl(_tier: PatreonTier): string {
...
export async function getAllPatreonTiers(): Promise<PatreonTier[]> {
  ...
  if (!campaignId || !accessToken) {
    // Patreon environment variables are missing. Skipping Patreon fetch.
    return [];
  }
  ...
  try {
    ...
    const tiers =
      data.included?.filter(
        (item: Record<string, unknown>) => item.type === "tier"
      ) || [];
    return tiers as PatreonTier[];
  } catch (_error) {
    // Error in getAllPatreonTiers
